/*
** EPITECH PROJECT, 2019
** CPool_Day09_2019
** File description:
** task02 my.h macro that contains the prototypes of libmy.a
*/

#ifndef MY_H_
#define MY_H_

void my_putchar(char);
int my_isneg(int);
int my_put_nbr(int);
void my_swap(int *a; int *b);
int my_putstr(char const *str);
int my_strlen(char const *str);
int my_getnbr(char const *str);
void my_sort_it_array(int *tab, int size);
int my_compute_power_rec(int nb, int power);
int my_compute_square_root(int);
int my_is_prime(int);
int my_find_prime_sup(int);
char *my_strcpy(char *dest, char const *src);
char *my_strncpy(char *dest, char const *src, int n);
char *my_revstr(char *str);
char *my_strstr(char *str, char const *to_find);
int my_strcmp(char const *s1, char const *s2);
int my_strncmp(char const *s1, char const *s2, int n);
char *my_strupcase(char *str);
char *my_strlowcase(char *str);
char *my_strcapitalize(char *str);
char my_str_isalpha(char const *str);
char my_str_isnum(char const *str);
char my_str_islower(char const *str);
char my_str_isupper(char const *str);
char my_str_isprintable(char const *str);
char my_str_showstr(char const *str);
char my_str_showmem(char const *str, int size);
char *my_strcat(char *dest, char const *src);
char *my_strncat(char *dest, char const *src, int nb);

#endif

